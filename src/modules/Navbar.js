...
const Navbar = () => {
  ...
  const [dropdownItemData, setDropdownData] = useState([])
  ...
  const HeaderLinks = () => {
    ...
    return <>{headerItems.map(({ label, href }, i) => (
      <li key={i}>
        <Link href={href} className={router.asPath == href ? selectedClass : regularClass + " block py-2 pl-3 pr-4 md:p-0"}>
          {label}
        </Link>
      </li>
    ))}</>
  ...
  return (
    <nav className="bg-white border-gray-200 px-2 sm:px-4 py-2.5 rounded dark:bg-gray-900 top-0 z-10 border-b-2">
      <div className="container flex flex-wrap items-center mx-auto justify-between">
        ...
        <Link href={title.href} className="flex items-center mr-3 md:hidden">
